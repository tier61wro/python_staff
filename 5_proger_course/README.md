...
- [Уроки](#Уроки)
    ...
    - [урок4](#урок4)
    - [урок5](#урок5)
    - [урок6  Циклы For, While, а также операторы](#урок6)
    - [урок7](#урок7)
    - [урок8](#урок8)
    - [урок9](#урок9)
    - [урок10](#урок10)
...
Циклы позволяют выполнять некий код несколько раз подряд. Это особенно полезно при работе с массивами (их мы будем учить немного позже). В языке Python циклы немного отличаются по своему виду от циклов в других языках. К примеру, во многих языках есть цикл do..while, который выполняется один раз сто процентов, а потом проверяет условие. Здесь же его нет, но это не является проблемой, так как можно спокойно написать свой алгоритм, который будет выполнять такую же задачу.

Не стоит создавать бесконечные циклы, так как это приведет к тому, что компьютер просто зависнет и перестанет работать. Также важно отметить, что вы можете создать бесконечный цикл, но с оператором break, который выйдет из цикла:
```python
a = 10
while True:
  a -= 1
